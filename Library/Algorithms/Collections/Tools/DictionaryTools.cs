...
namespace Softperson.Collections
{
	...
	{
		[DebuggerStepThrough]
		public static Dictionary<K, V> Clone<K, V>(IDictionary<K, V> dict)
		{
		...
		{
			var hashcode = pair.Key.GetHashCode();
			if (pair.Value != null)
		...
		{
			var v = func(Get(dict, key, key2, key3));
		...
		{
			var v = func(Get(dict, key, key2, key3, key4));
			Set(dict, key, key2, key3, key4, v);
		}

		[DebuggerStepThrough]
		public static Dictionary<K, V> New<K, V>()
		{
			return new Dictionary<K, V>();
		}

		[DebuggerStepThrough]
		public static Dictionary<K, Dictionary<K2, V>> New<K, K2, V>()
		{
			return new Dictionary<K, Dictionary<K2, V>>();
		}

		[DebuggerStepThrough]
		public static Dictionary<K, Dictionary<K2, Dictionary<K3, V>>> New<K, K2, K3, V>()
		{
			return new Dictionary<K, Dictionary<K2, Dictionary<K3, V>>>();
		}

		[DebuggerStepThrough]
		public static Dictionary<K, Dictionary<K2, Dictionary<K3, Dictionary<K4, V>>>> New<K, K2, K3, K4, V>()
		{
			return new Dictionary<K, Dictionary<K2, Dictionary<K3, Dictionary<K4, V>>>>();
		}
		...
		public static int GetHashCode<K, V>(IDictionary<K, V> dictionary, bool checkValues)
		{
			...
			{
				var tmp = pair.Key.GetHashCode();
				if (checkValues && pair.Value != null)
					tmp = Utility.CreateHashCode(tmp, pair.Value.GetHashCode());
				hashcode ^= tmp;
			}
			return hashcode;
		}
		...
		{
			return dict.Force(key).Increment(key2, value);
		}

		public static int Increment<T, T2, T3>(this Dictionary<T, Dictionary<T2, Dictionary<T3, int>>> dict, T key, T2 key2,
			T3 key3, int value = 1)
		{
			return dict.Force(key).Force(key2).Increment(key3, value);
		}

		public static int Decrement<T>(this Dictionary<T, int> dict, T key)
		{
			return Increment(dict, key, -1);
		}
		...
		public static V Force<T1, T2, V>(this Dictionary<T1, Dictionary<T2, V>> dict, T1 key, T2 key2)
			where V : new()
		{
			return Force(Force(dict, key), key2);
		}
		...
		public static V Force<T1, T2, T3, V>(this Dictionary<T1, Dictionary<T2, Dictionary<T3, V>>> dict, T1 key, T2 key2,
			...
			where V : new()
		{
			return Force(Force(Force(dict, key), key2), key3);
		}

...
namespace Softperson.Collections
{
	...
	public static class ListTools
	{
		...
		///     Specifically, std::rotate swaps the elements in the range[start, start+count) in such a way that the element
		...
		public static void StableSort2<T>(this T[] list, Comparison<T> comparison)
		{
			...
			Array.Sort(indices, array,
				CompareTools.ToComparer<int>((a, b) =>
				{
					var cmp = comparison(list[a], list[b]);
					return cmp != 0 ? cmp : a.CompareTo(b);
				}));
		...
		{
			list.Sort(keyFunc.ToComparison());
		}
...
#line default
		...
		public static T[] CopyRange<T>(this IList<T> list, int position, int count)
		{
			...
			if (count == 0)
				return ArrayTools.EmptyArray<T>();
		...
		{
			if (count == 0) return ArrayTools.EmptyArray<T>();
			var array = new T[count];
...
// ReSharper restore CompareNonConstrainedGenericWithNull
		...
		{
			return AllSame(list, FuncTools.Identity<T>());
		}

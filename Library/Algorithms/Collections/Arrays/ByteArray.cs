...
namespace Softperson.Collections
{
	...
	public class ByteArray
	{
		...
		public static explicit operator ByteArray(int value)
		{
			return new ByteArray(BitConverter.GetBytes(value));
		}
		...
		public static explicit operator ByteArray(double value)
		{
			return new ByteArray(BitConverter.GetBytes(value));
		}

		public static explicit operator ByteArray(long value)
		{
			return new ByteArray(BitConverter.GetBytes(value));
		}

		public static explicit operator ByteArray(short value)
		{
			return new ByteArray(BitConverter.GetBytes(value));
		}
		...
		public byte Index1(int index)
		{
			return _data[_offset + index];
		}

		public short Index2(int index)
		{
		...
		{
			return BitConverter.Int64BitsToDouble(ReadLong(8));
		}
		...
		{
			var data1 = _data;
			var data2 = ((ByteArray) x)._data;
			if (data1 == data2)
				return true;
			var cmp = data1.Length - data2.Length;
			if (cmp != 0)
				return false;
...
// ReSharper disable NonReadonlyFieldInGetHashCode
			var hash = _data.Length;
			for (var i = 0; i < _data.Length; i++)
			{
				var tmp = _data[i].GetHashCode();
				tmp |= tmp << 8;
				tmp |= tmp << 16;

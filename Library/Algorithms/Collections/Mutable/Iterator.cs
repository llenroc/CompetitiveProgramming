...
namespace Softperson.Collections
{
		...
		where T : class, IIndexable<T>
	{
		...
		{
			var common = level > iterator.level ? iterator.level : level;

			for (var i = 0; i < common; i++)
			{
				var cmp = indices[i] - iterator.indices[i];
				if (cmp != 0)
				{
					maxLevel = i;
					return cmp;
		...
		public static bool operator ==(Iterator<T> context1, Iterator<T> context2)
		{
		...
		public static bool operator <=(Iterator<T> context1, Iterator<T> context2)
		{
			return context1.CompareTo(context2) <= 0;
		}
		...
		private void Push(T e, int pos)
		{
			if (level == indices.Length)
			{
				var newLength = level*2;
		...
		public T Current
		{
			get { return expressions[level - 1]; }
		}
		...
		public int Index
		{
			get
			{
				Utility.Assert(indices[level - 1] >= 0, "Index is < 0");
		...
		public bool AtStart
		{
			get { return level == 1 && indices[0] == 0; }
		}

		public bool AtEnd
		{
			get { return level == 1 && indices[0] >= expressions[0].Count; }
		}

...
namespace Softperson.Collections
{
	partial class Rope<T>
	{
		...
		public class List : IList<T>
		{
			...
			public int Count
			{
				...
				set
				{
					...
					if (count > value)
						...
						Insert(count, value, default(T));
				}
			}

			[NotNull]
			public Rope<T> Rope { get; private set; }
			...
			{
				return Rope.PreviousRun(position);
			}
			...
			/// </summary>
			[NonSerialized] private Rope<T> _mruNode;

			[NonSerialized] private int _mruStart;
			...
			public T this[int index]
			{
				...
				{
					var n = _mruNode;
					if (n == null || (uint) (index - _mruStart) >= (uint) n.Count)
					{
			...
			/// <returns></returns>
			public int IndexOf(T value, int start, int count = int.MaxValue)
			{
				return Rope.IndexOf(value, start, count);
			}
			...
			public void InsertRange(int index, IEnumerable<T> list)
			{
				...
				var vector = slist != null ? slist.Rope : FromX(list);
				Insert(index, vector);
			}
			...
			private void Insert(int index, Rope<T> insertion)
			{
				Debug.Assert(Rope != null);
			...
			public Rope<T> Copy(int start, int count)
			{
				...
				return Rope.Copy(start, count);
			}

			public Rope<T> Cut(int start, int count)
			{
				var cut = Copy(start, count);
				RemoveRange(start, count);
				return cut;
			}
			...
			public static implicit operator Rope<T>(List builder)
			{
				return builder.ToVector();
			}

			public Rope<T> ToVector()
			{
				return Rope.Seal();
			}

...
namespace Softperson.Collections
{
		...
		where T : Treap<K, T>
	{
		...
		public static void Seal(Treap<K, T> top)
		{
			...
			if (top._left != null)
			{
				Utility.Assert(top._left._priority <= top._priority);
				Seal(top._left);
			...
			if (top._right != null)
			{
				Utility.Assert(top._right._priority <= top._priority);
				Seal(top._right);
		...
		public static bool IsSealed(Treap<K, T> top)
		{
			...
			Utility.Assert(!isSealed || top._left == null || top._left._flags >= UnchangeableBit);
			Utility.Assert(!isSealed || top._right == null || top._right._flags >= UnchangeableBit);
			return isSealed;
		}
		...
		[DebuggerNonUserCode]
		public virtual int NodeCompare(T node2)
		{
			return 0;
		}
		...
		[DebuggerNonUserCode]
		protected virtual bool NodeEquals(T map)
		{
			return true;
		}

		[DebuggerNonUserCode]
		protected virtual int NodeGetHashCode()
		{
			return 0;
		}
		...
		{
			return CompareKey(key, key.GetHashCode());
		}
		...
		protected virtual int OnCompareKey(K key)
		{
			return Comparer<K>.Default.Compare(Key, key);
		}
		...
		protected static T CloneLeft(T self, T left)
		{
			...
			Utility.Assert(left == null || self._priority >= left._priority);
			self._left = left;
			return self;
		}
		...
		protected static T CloneRight(T self, T right)
		{
			...
			Utility.Assert(right == null || self._priority >= right._priority);
			self._right = right;
			return self;
		}
		...
		{
			Utility.Assert(_left == null || _left._priority <= _priority);
			Utility.Assert(_right == null || _right._priority <= _priority);
			if (IsSealed(this))
			{
				Utility.Assert(IsSealed(_left));
				Utility.Assert(IsSealed(_right));
			}
		}
		...
		public static T Insert(T top, T insertion)
		{
			...
			else
			{
				if (top.NodeEquals(insertion))
					return top;
				Utility.Assert(!IsSealed(insertion));
		...
		public static T Clone(T top, T leftNode, T rightNode)
		{
			...
			Utility.Assert(leftNode == null || self._priority >= leftNode._priority);
			Utility.Assert(rightNode == null || self._priority > rightNode._priority);
			self._left = leftNode;
...
#line default
		...
		public List<T> Nodes
		{
			get { return new List<T>(Select((T) this, node => node)); }
		}
		...
		public static T Union(T map1, T map2)
		{
			...
			if (map1._priority <= map2._priority)
				Utility.Swap(ref map1, ref map2);
		...
		private static T Pop(Stack<T> stack)
		{
			if (stack.Count == 0)
				return null;
			return stack.Pop();
		...
		public static bool Overlaps(T set1, T set2)
		{
			...
			if (Count(set1) > Count(set2))
				Utility.Swap(ref set1, ref set2);
		...
		[DebuggerStepThrough]
		public static T Difference(T map1, T map2)
		{
			return Diff(map1, map2, true);
		}

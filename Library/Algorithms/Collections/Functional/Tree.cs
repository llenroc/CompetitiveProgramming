		...
		where TNode : Tree<TData, TNode>
	{
		...
		public abstract class BalancingAlgoritm
		{
			...
			public TNode Insert(TNode tree, TData data)
			{
				...
				var cmp = comparer.Compare(data, tree.Data);
				TNode result;
			...
			private bool IsBalanced(TNode left, TNode right)
			{
			...
			private int Measure(TNode child)
			{
				return child != null ? child.Height : 1;
			}
			...
			public TNode FindExtremum(TNode tree, bool minimum)
			{
				while (tree != null)
				{
					var node = minimum ? tree.Left : tree.Right;
					if (node == null)
						return tree;
					tree = node;
				}
			...
			public TNode Find(TNode top, TData data)
			{
				...
				while (current != null)
				{
					var cmp = comparer.Compare(data, current.Data);
					if (cmp < 0)
			...
			public TNode Intersection(TNode node1, TNode node2)
			{
				...
				var n = Split(node2, node1.Data, out splitL, out splitR);

...
namespace Softperson.Algorithms.Graphs
{
    ...
    public class FordFulkersonMinCut
    {
        ...
        int _s;
        int _t;
        List<Tuple<int, int>> Cuts = new List<Tuple<int, int>>();
  ...
  residual graph. Also fills parent[] to store the path */
        bool Bfs(int[,] rGraph, int s, int t, int[] parent)
        {
            ...
            BitArray visited = new BitArray(_v);
        ...
        public FordFulkersonMinCut(int[,] graph, int s, int t)
        {
            ...
            // in residual graph
            var rGraph = new int[_v, _v]; // rGraph[i][j] indicates residual capacity of edge i-j
            for (u = 0; u < _v; u++)
                for (v = 0; v < _v; v++)
                    rGraph[u, v] = graph[u, v];

            var parent = new int[_v];  // This array is filled by BFS and to store path
        ...
        int main()
        {
            ...
            foreach (var cut in minCut.Cuts)

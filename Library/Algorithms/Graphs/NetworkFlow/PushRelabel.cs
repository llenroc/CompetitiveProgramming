...
namespace Softperson.Algorithms.Graphs
{
    ...
    public class PushRelabel
    {
        ...
        {
            var amt = (int)Math.Min(_excess[e.From], e.Cap - e.Flow);
        ...
        {
            for (var i = 0; _excess[v] > 0 && i < _g[v].Count; i++) Push(_g[v][i]);
            if (_excess[v] > 0)
            {
                if (_count[_dist[v]] == 1)
                    Gap(_dist[v]);
        ...
        public long GetMaxFlow(int s, int t)
        {
            ...
            long totflow = 0;
            for (var i = 0; i < _g[s].Count; i++) totflow += _g[s][i].Flow;
            return totflow;
        }

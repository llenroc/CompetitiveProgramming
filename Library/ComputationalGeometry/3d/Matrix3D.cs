...
namespace Softperson.ComputationalGeometry
{
    ...
    public class Matrix3D
    {
        ...
        private Matrix3D(Matrix3D matrix1, Matrix3D matrix2)
        {
            AssignProduct(matrix1, matrix2);
        }

        private Matrix3D(bool flag)
        {
        }

        private Matrix3D(double diagonal)
        {
            E11 = E22 = E33 = diagonal;
        }

        public Matrix3D Clone()
        {
            return (Matrix3D) MemberwiseClone();
        }

        public static Matrix3D GetDiagonal(double d)
        {
        ...
        {
            get { return Numbers.IsZeroed(Determinant); }
        }
        ...
        {
            get { return Math.Abs(Determinant).AreClose(1); }
        }
        ...
        private void AssignProduct(Matrix3D m1, Matrix3D m2)
        {
            if (m2.IsIdentity)
            {
                Assign(m1);
        ...
        public static Matrix3D RotationMatrix(Vector3D axis, double angle,
                                              Point3D center)
        {
            return RotationMatrix(new Quaternion(axis, angle), center);
        }
        ...
        public static Ray3D operator *(Ray3D ray, Matrix3D matrix)
        {
            return new Ray3D(ray.Point*matrix, ray.Vector*matrix);
        }

...
namespace Softperson
{
	...
	public static class StringTools
	{
		...
		public static string PrefixByteOrderMark(string s)
		{
			...
			var byteOrderMark = Encoding.Unicode.GetString(preamble, 0, preamble.Length);
		...
		public static string Normalize(string s, bool removeSpaces = true)
		{
			...
			var write = 0;
			for (var read = 0; read < s.Length; read++)
			{
				var ch = s[read];
		...
		public static long RemoveNumber(StringBuilder builder)
		{
			...
			var start = Math.Max(0, builder.Length - 18);

			var factor = 1;
			while (index >= 0)
			{
				var d = builder[index]-'0';
				if (d < 0 || d > 9)
					break;
			...
			out int start, out int charsFromEnd, bool ignoreCase = false)
		{
			...
			var last1 = string1.Length - 1;
			var last2 = string2.Length - 1;
			var count = Math.Min(last1, last2) + 1;
			if (count == 0)
				return false;
			...
			for (e = 0; e < count; e++)
			{
				var ch1 = string1[last1 - e];
				var ch2 = string2[last2 - e];
		...
		private class TextFixer
		{
			private readonly Dictionary<int, int> _u2a;

			private string _text;

			public TextFixer(int codepage)
			{
				var ansi = new byte[32];
				for (var i = 0; i < 32; i++)
					ansi[i] = (byte) (i + 128);

				var win1252 = Encoding.GetEncoding(codepage);
				var converted = win1252.GetString(ansi);
			...
			private bool ProcessChar(int i, out int shift, out int value)
			{
				value = 0;
				shift = 0;
			...
			private bool ReadPseudo(int i, out int result)
			{
				if (i >= _text.Length)
				{
					result = 0;
			...
			private bool ReadContinuation(int i, out int result)
			{
				return ReadPseudo(i, out result) && ((result & 0xc0) == 0x80);
			}
		}

...
namespace Softperson
{
	public static class BitTools
	{
		...
		public static int BranchlessSignExtend(int n)
		{
			return n >> 31;
		}

		public static long BranchlessSignExtend(long n)
		{
			return n >> 63;
		}
		...
		{
			return unchecked((long)RotateRight((ulong)bits, n));
		}
		...
		public static long LowestOneBit(long n)
		{
			return n & -n;
		}
		...
		public static int ShiftRight(int n, int shift = 1)
		{
			return (int)((uint)n >> shift);
		}

		public static long ShiftRight(long n, int shift = 1)
		{
			return (long)((ulong)n >> shift);
		}
		...
		static class DeBruijnUntested
		{
			...
			public static int TrailingZeros(int number)
			{
				...
				return (int)((low >> (number << 1) & 3) | (med >> (number << 1) & 3) << 2 | (hi >> number & 1) << 4);
			}
		}
...
public class CarryLessMultiplication
{
	...
	public static void Set(ulong[] bitset, int d) => bitset[d >> 6] |= 1ul << (d & 63);

	public static void Clear(ulong[] bitset, int d) => bitset[d >> 6] &= ~(1ul << (d & 63));
}

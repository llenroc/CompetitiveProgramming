...
namespace Softperson.Mathematics
{
	...
	{
		public static int ModulusInverse(int x, int mod)
		{
			return (int)ModulusInverse(x, (long)mod);
		}

		public static long ModulusInverse(long x, long mod)
		{
			long xFactor, modFactor;
			var gcd = ExtendedGcd(x, mod, out xFactor, out modFactor);
			Debug.Assert(gcd == 1);

			var inverse = Modulus(xFactor, mod);
		...
		public static int Modulus(int x, int mod)
		{
			return (int)Modulus((long)x, mod);
		}
		...
		{
			long xFactor2, yFactor2;
			var gcd = ExtendedGcd(x, y, out xFactor2, out yFactor2);
		...
		public static int GetPrime(int minSize)
		{
			...
			for (var j = minSize | 1; j < int.MaxValue; j += 2)
			{
				if (IsPrime(j))
					return j;
			}
		...
		{
			var pow = ModulusInverse(root, mod - 1);
		...
		public static int ModulusPow(int n, int pow, int mod)
		{
			...
			var result = ModulusPow(n, pow / 2, mod);
		...
		public static double Gcd(double d1, double d2)
		{
			if (!(d1 >= 0))
			{
				if (d1 < 0) d1 = -d1; else return 1;
			}

			if (!(d2 >= 0))
			{
				if (d2 < 0) d2 = -d2; else return 1;
			}

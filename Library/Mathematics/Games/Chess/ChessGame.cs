			...
			foreach (var token in Parse(pgn))
			{
				switch (token[0])
				{
					case '"':
						if (tag != null && tagMode)
						{
							var s = CodeTools.DecodeString(token.Substring(1, Math.Max(0, token.Length - 2)));
							this[tag] = s;
							tag = null;
						}
						break;
		...
		public string SavePgn(bool emitTags = true)
		{
			...
			if (emitTags)
			{
				...
				foreach (var item in set)
				{
					...
					sb.Append(item);
					sb.Append(' ');
					sb.Append(CodeTools.Quote(this[item]));
					sb.Append("]\n");
				}

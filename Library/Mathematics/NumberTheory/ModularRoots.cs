...
using static Softperson.Mathematics.FactorizationSingle;
    ...
    public static class ModularRoots
    {
		...
		public static int PrimitiveRoot(int n)
        {
            ...
            for (int a = 2; a < n; a++)
            {
                ...
                {
                    var pow = ModularMath.ModPow(a, s / prime, n);
                    if (pow == 1)
                    {
                        good = false;
                        break;
	    ...
	    public static bool IsPrimitive(long g, long n)
	    {
		    ...
		    var orderfacts = Factorize(order);
		    long oldfact = 1;
		    foreach (var fact in orderfacts)
		    {
			    if (fact != oldfact)
			    {
	    ...
	    static long TSRsqrtmod(long a, long grpord, long p)
	    {
		    ...
		    long tmp = ModularMath.ModPow(a * ModularMath.ModPow(g, gpow, p), d, p);  // sqrt(a*(g**gpow))

...
using static Softperson.Mathematics.NumberTheory;
...
namespace Softperson.Mathematics
{
	public class FactorizationSingle
	{
		...
		{
			var thefactors = Factorize(n).ToList();
			thefactors.Sort();
			...
			long oldfact = 1;
			foreach (var fact in thefactors)
			{
				if (fact == oldfact)
		...
		/// <remarks>https://en.wikipedia.org/wiki/Pollard's_rho_algorithm.
		/// Additional optimizations available.</remarks>
		/// <returns></returns>
		...
		/// <returns></returns>
		public static long CountDivisors(long rParam)
		{
			long n = rParam;
			if (MillerRabin.IsPrime(n))
				return 1;
			...
			if (sqrt * sqrt == n && MillerRabin.IsPrime(sqrt))
			{
				// NOTE: The IsPrime test might not be necessary
				answer *= 3;
			}

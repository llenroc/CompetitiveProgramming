...
namespace Softperson.Mathematics
{
	...
	public class DualNumber
	{
		...
		public static DualNumber Cos(DualNumber a) => new DualNumber(

			Math.Cos(a.Real),
			-a.Dual * Math.Sin(a.Real)
		);

		public static DualNumber Tan(DualNumber a) => new DualNumber(

			Math.Tan(a.Real),
			a.Dual / (Math.Cos(a.Real) * Math.Cos(a.Real))
		);

		public static DualNumber Atan(DualNumber a) => new DualNumber(
			Math.Atan(a.Real),
		...
		public static DualNumber SmoothStep(DualNumber x)
		{
			// f(x) = 3x^2 - 2x^3
			// f'(x) = 6x - 6x^2
			return x * x * (new DualNumber(3) - new DualNumber(2) * x);
		}

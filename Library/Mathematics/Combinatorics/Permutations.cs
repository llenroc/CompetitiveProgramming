...
namespace Softperson.Mathematics
{
	public static partial class Permutations
	{
		...
		public static int GetRank(string word)
		{
			...
			var charsSorted = word.ToCharArray().OrderBy(x => x).ToArray();
		...
		{
			var clone = array.ToArray();
			Array.Sort(clone);
			Comparison<T> comparison = Comparer<T>.Default.Compare;
			do
				yield return clone; while (NextPermutation(clone, comparison));
		}
		...
		public static bool NextPermutation<T>(T[] array, Comparison<T> comparison = null)
		{
			...
			{
				var mid = (min + max) >> 1;
				var cmp = comparison(pivot, array[mid]);
				if (cmp >= 0)
					min = mid + 1;
				else
					max = mid - 1;
		...
		{
			private readonly Action<List<int>> _action;
			private readonly List<int> _tempList;
			private readonly int[] _nums;
			private readonly bool _handleDupes;
			...
			private void BacktrackSubset(int start = 0)
			{
				_action(_tempList);
		...
		public class PermutationIterator
		{
			...
			private void BacktrackPermute(long used = 0)
			{
		...
		public class CombinationIterator
		{
			...
			private void BacktrackCombination(int start = 0)
			{
		...
		public class CombinationSumIterator
		{
			...
			private void BacktrackCombination(int remain, int start = 0)
			{
				if (remain < 0) return;
		...
		public static long InitCombination(int n, int k)
		{
			return ((1L << k) - 1) << (n - k);
		}
		...
		public static void ShuffleCycle<T>(this IList<T> list, int start, int count)
		{
			...
			for (var i = start; i + 1 < n; i++)
			{
				var j = random.Next(i+1, n); // random # from [i+1,n)
				var tmp = list[i];
				list[i] = list[j];
				list[j] = tmp;
			}
		...
		public static int[] ShuffleDerangement(int n)
		{
			...
			for (var i = 0; i + 1 < n; i++)
			{
				var j = r.Next(i, n); // random # from [i,n)
				var tmp = derangement[j];
				if (tmp != i)
				{

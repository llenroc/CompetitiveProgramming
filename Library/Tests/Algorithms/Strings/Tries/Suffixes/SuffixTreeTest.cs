...
namespace Softperson.Algorithms.Strings
{
	...
	[TestFixture]
	public class SuffixTreeTest : Test
	{
		#region Variables
		...
		void CheckSuffixArray(string s)
        {
            ...
            var suffixArray = suffixTree.GetSuffixArray();
        ...
        public static void CheckSuffixArray(string s, int[] suffixArray, int extra=0)
        {
            ...
            var prev = s.Substring(suffixArray[0]);
            for (int i = 1; i < suffixArray.Length; i++)
            {
                var cur = s.Substring(suffixArray[i]);
        ...
        void CheckIndices(string text, params string[] pattern)
        {
            ...
            {
                var list = tree.IndicesOf(p).ToList();
        ...
        void TestLongestRepeatedSubstring(string s)
        {
            ...
            var ci = StringTools.CountInstancesOf(s, s.Substring(start, length));

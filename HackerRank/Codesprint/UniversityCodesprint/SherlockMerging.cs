...
namespace HackerRank.UniversityCodesprint2.SherlockMerging
{
    using System;
    ...
    public class Solution
    {
        ...
        void Run()
        {
            ...
            _array = Array.ConvertAll(ReadLine().Split(), int.Parse);
            _longestLength = new int[n];

            for (int len=0, index=n-1; index >= 0; index--)
            {
                if (len > 0 && _array[index] > _array[index + 1])
                    len = 0;
                _longestLength[index] = ++len;
            }


            int maxParts = _longestLength[0];
            Slack = Min(maxParts, 0);

            _dp = new int[_array.Length + 1, maxParts + 1, Slack];
        ...
        long Compute(int length, int partitionsParam)
        {
            ...
            int index = _array.Length - length;
            partitions = Min(partitions, _longestLength[index]);
            ...
            if (slack <Slack)
            {
                if (_dp[index, partitions, slack] != 0)
                    return _dp[index, partitions, slack] - 1;
            }
            ...
            for (int i = 1; i <= partitions; i++)
            {
                ...
                answer = (answer + multiplied) % MOD; 

                var tmpSlack = partitionsParam - i;
                if (tmpSlack < 3)
                    _dp[index, i, tmpSlack] = (int) answer + 1;
            }

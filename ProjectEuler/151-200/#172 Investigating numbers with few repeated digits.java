...
public class G {
	...
	void solve()
	{
        ...
        long inv910 = 9 * pow(10, MOD-2, MOD) % MOD;

        long[] x = new long[m + 1];
        for (int i = 0; i <= m; i++) x[i] = ifact[i];

        //long[] x2 = convolute(x, x, 3, MOD);
        //long[] x3 = convolute(x2, x, 3, MOD);
        //long[] x5 = convolute(x3, x2, 3, MOD);
        //long[] x10 = convolute(x5,x5, 3, MOD);
        long [] x2 = exponentiate(x, 5, 10, MOD);
        long [] x10 = exponentiate(x2, 2, 3, MOD);
        
        while (t-->0)
        {
            int n = ni();
			out.println(inv910 * x10[n] % MOD * fact[n] % MOD );            
        }
	}
	
	long[] ans;
	
	public static long[] mul(long[] a, long[] b, int P, int g)
	{
		if(Math.max(a.length, b.length) >= 1000){
			return convoluteSimply(a, b, P, g);
		}else{
			return mulnaive(a, b);
		}
	}
    
    public static long ConvolutionTerm(long[] a, long[] b, int term)
	{
         int mod = MOD;
		if (a.length > b.length)
			return ConvolutionTerm(b, a, term);

		long sum = 0;
		for (int i = Math.min(a.length - 1, term); i >= 0; i--)
		{
			int j = term - i;
			if (j >= b.length) break;
			sum += a[i] * b[j] % mod;
...
//	public static final int[] NTTPrimitiveRoots = {7, 6, 3, 5, 3, 3, 3, 3, 3, 17};
	...
	public static long[] convolute(long[] a, long[] b)
	{
    	...
    	public static long[] exponentiate(long[] a, int n, int USE, int mod)
	{
		int m = Math.max(2, Integer.highestOneBit((a.length-1)*n)<<1);
		long[][] fs = new long[USE][];
		for(int k = 0;k < USE;k++){
			int P = NTTPrimes[k], g = NTTPrimitiveRoots[k];
			long[] fa = nttmb(a, m, false, P, g);

...
class Solution {
	public void solve()
	{
		...
		var factors = DivisorsUpTo(max);

		int[] table = new int[max + 1];
		for (int i = 2; i <= max; i++)
		{
			var c = 0;
			foreach (var f in factors[i])
			{
				var f2 = i / f;
				if (f2 > f || (f2+f & 1) == 1) continue;
				var b = (f2 + f)/2;
				var a = f - b;
				if (((a ^ b) & 1) == 1 || a==0) continue;
				c++;
				if (c > 10) break;
			}

			bool add = c >= 1 && c <= 10;
			//if (add) Console.Error.WriteLine($"Added {i} with {c}");
			table[i] = table[i - 1] + (add ? 1 : 0);
		}

...
class Solution
{
	public void solve()
	{
		...
		var targets = Ni(t);
		var max = targets.Max();
		var table = new long[max*2];
		var diff = new int[table.Length];

		for (int i = 1; i < 2000; i++)
		{
			long ci = i * (i + 1) >> 1;
			if (ci > table.Length) break;

			for (int j = i; j < 2000; j++)
			{
				int cj = j * (j + 1) >> 1;
				long rects = ci * 1L * cj;
				if (rects >= table.Length) break;
				long area = j * 1L * i;
				table[rects] = Math.Max(area, table[rects]);
			}
		}
		...
		for (int i = table.Length - 2; i > 0; i--)
		{
			if (diff[i] == 0)
				lastIndex = i;
			else if (diff[i] > lastIndex-i || diff[i]==lastIndex-i && table[lastIndex]>table[i])
			{
				if (diff[lastIndex] == 0)
				{
					table[i] = table[lastIndex];

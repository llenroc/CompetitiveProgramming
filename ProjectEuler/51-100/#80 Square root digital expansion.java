...
public class Solution
{
	...
	private static BigInt upper = new BigInt(0);
	private static BigInt lower = new BigInt(0);
	...
	public static void main(String[] args)
	{
		...
		BigInt[] table = new BigInt[10001];
        Scanner scanner = new Scanner(System.in);
		int n = Integer.parseInt(scanner.nextLine());
		int p = Integer.parseInt(scanner.nextLine());

		int pp = p + 5;
		BigInt b2p = pow(10, 2* pp);
		BigInt bp = pow(10, pp);
		BigInt bp15 = pow(10, pp-15);
		...
		for (int i = 2; i <= n; i++)
		{
            ...
            if ( factors[i] > 1 )
            {
				sqrt = table[i / factors[i]].copy();
                sqrt.mul(table[factors[i]]);
            }
            else
            {
	            BigInt x = b2p.copy();
				x.mul(i);
                lower = new BigInt((long)(sqd * 100000L * 100000L * 100000L));
				lower.mul(bp15);
                sqrt = Sqrt2(x);
            }
    ...
    static BigInt pow(long a, long b) {
    ...
    public static BigInt Sqrt(BigInt x)
    {
        if (x.isZero())
            return x;

        BigInt start = lower.copy();
        BigInt end = lower.copy();
        start.shiftRight(1);
        end.shiftLeft(1);
        ...
        while (start.compareTo(end)<0)
        {
            ...
            BigInt mid = start.copy();
            mid.add(end);
            mid.shiftRight(1);
            BigInt sqr = mid.copy();
            sqr.mul(mid);

            int cmp = sqr.compareTo(x);
            if (cmp==0) return mid; 
            if (cmp< 0)
            {
                mid.add(1);
    ...
    public static BigInt Sqrt2(BigInt n)
    {
        if (n.isZero()) return n;
        BigInt root = lower.copy();
        BigInt upper = new BigInt(0);
        while (true)
        {
            BigInt root2 = root.copy();
			root2.mul(root);
            if ( n.compareTo(root2)>=0 )
            {
				upper.assign(root);
				upper.shiftLeft(1);
				upper.add(root2);
                if (n.compareTo(upper) <= 0) break;
            }

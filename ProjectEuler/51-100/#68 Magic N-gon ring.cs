...
class Solution
{
	...
	public void solve()
	{
		...
		{
			outer[0] = i;
			for (int a = 1; a <= 2 * n && i + a < s; a++)
			{
				int b = s - i - a;
				if (a == i || b == i || a == b || b > 2 * n) continue;
				inner[0] = a;
				inner[1] = b;
				Dfs((1 << i) | (1 << a) | (1 << b), 1, i + 1);
			}
		}
	...
	void Dfs(int maskOrig, int index, int min)
	{
		if (index == n)
		{
		...
		{
			if ((maskOrig & 1 << i) != 0) continue;
			outer[index] = i;

			int a = inner[index];
			int b = s - i - a;
			if (b <= 0 || b > 2*n) continue;

			int mask = maskOrig | 1 << i;
			if ((mask & 1 << b) != 0 && index+1<n) continue;
			mask |= 1 << b;

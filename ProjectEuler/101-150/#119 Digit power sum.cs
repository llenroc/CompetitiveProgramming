...
class Solution {
	public void solve()
	{
		...
		var maxdigits = Math.Ceiling(100 * Math.Log(10, b));
		var maxsum = (int)(b * maxdigits);

		var limit = BigInteger.Pow(10, 100);
		var list = new HashSet<BigInteger>();

        var seen = new HashSet<BigInteger>();

        for (long k = 2; k <= maxsum; k++)
		{
            //if (check[k]) continue;
			//for (long c = k*k; c <= maxsum; c *= k)
            //    check[c] = true;

            int p = 1;
			for (BigInteger c = k; c < limit; c *= k, p++)
			{
				if (c>=b && SumDigits(c,b,k))
					list.Add(c);
			}
		}
	...
	bool SumDigits(BigInteger n, int b, long k)
	{
		int sum = 0;
        var b2 = b*b;

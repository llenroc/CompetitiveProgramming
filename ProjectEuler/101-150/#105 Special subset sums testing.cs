...
class Solution {
	...
	public bool Check(int[] a)
	{
		...
		// Check that minimum subset of size x+1 is greater than maximum subset of size x
		...
		{
			long min = sums[i];
			long max = sums[n] - sums[n - i];
			long combos = Comb(n, i);
			if (combos > max - min + 1)
				return false;
		}

		return Search(new HashSet<long>(), a, 0, 0);
	}

	bool Search(HashSet<long> hash, int[] a, int index, long sum)
	{
		for (; index < a.Length; index++)
		{
			var sum2 = sum + a[index];
			if (!hash.Add(sum2))
				return false;

			if (!Search(hash, a, index + 1, sum2))
				return false;
		}
	...
	private const int FactSize = 2000001;

	public static long Fact(int n)
	{
		if (_fact == null) _fact = new List<long>(FactSize) { 1 };
	...
	{
		if (_ifact == null) _ifact = new List<long>(FactSize) { 1 };
		for (int i = _ifact.Count; i <= n; i++)
			_ifact.Add(Mult(_ifact[i - 1], Inverse(i)));
		return _ifact[n];
	}
